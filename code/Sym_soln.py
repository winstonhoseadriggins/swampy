...
class DensePoly(Poly):
    ...
    def __mul__(self, other):
        ...
        if self.var != other.var:
            raise ValueError, 'Cannot mult polynomials of different variables.'

        other = other.convert_dense()
        res = SparsePoly(self.var, {0:0})
        for i, ci in enumerate(self.coefs):
            for j, cj in enumerate(other.coefs):
                exp = i + j
                coef = ci * cj
                res += SparsePoly(self.var, {exp:coef})
        return res.convert_dense()
...
def main(script, *args):
    ...
    # add a few lines below to create two SparsePolys, then
    # compute and print their sum
    p5 = SparsePoly(x, {0:1, 2:3})
    p6 = SparsePoly(x, {2:1, 4:3})
    print p5 + p6
...
#  1) Refactor so that __str__ is in the parent class.

#  2) Refactor so that coeff_sum is in the parent class.
